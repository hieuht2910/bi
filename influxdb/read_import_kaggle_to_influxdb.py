...
def create_spark_session():
    ...
    return (SparkSession.builder
            .appName("HDFS to InfluxDB Importer")
...
def transform_data(df):
    ...
    try:
        # divide column by 20
        df = df.withColumn("Open", F.col("Open") / 20)
        df = df.withColumn("High", F.col("High") / 20)
        df = df.withColumn("Low", F.col("Low") / 20)
        df = df.withColumn("Close", F.col("Close") / 20)
        df = df.withColumn("Adj Close", F.col("Adj Close") / 20)
